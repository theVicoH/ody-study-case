...
import { useIsMobile } from "@workspace/ui/hooks/use-mobile/use-mobile.hook";
...
const SIDEBAR_COOKIE_MAX_AGE = SECONDS_IN_MINUTE * MINUTES_IN_HOUR * HOURS_IN_DAY * DAYS_IN_WEEK;
    ...
    >
      ...
      >
        <div
          ...
          className="bg-sidebar group-data-[variant=floating]:ring-sidebar-border flex size-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 dark:backdrop-blur-xl dark:backdrop-saturate-150"
        >
          {children}
        </div>
      </div>
...
const SKELETON_WIDTH_RANGE_PCT = 40;
...
function SidebarMenuSkeleton({
  ...
  const [width] = React.useState(() => {
    return `${Math.floor(Math.random() * SKELETON_WIDTH_RANGE_PCT) + SKELETON_MIN_WIDTH_PCT}%`;
  });
